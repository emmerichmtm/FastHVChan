...
covering `chan_hypervolume.py` and `hv_baselines.py`.

The C code is not well tested yet, so please use the Python code as the
reference. The Chan port has been checked against brute-force
inclusion-exclusion and against random cases replayed from the Python version.
The two baseline ports (dimension sweep and WFG) have not been compiled or run
at all: they were written on a machine with no C compiler. They come with a
test suite (`test_hv_baselines`, run by `make test`) meant to catch porting
mistakes, but that suite has not been run either. It is worth running
`make test` before relying on the C code. See `c/README.md`.
