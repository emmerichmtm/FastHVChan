...
the second specialized to the grounded orthants that arise in hypervolume
computation.  We walk through both algorithms in an implementation-oriented
...
\item \texttt{chan\_orthant\_dby3.py} --- Chan's Section-4.2 orthant
  algorithm, specialized to grounded orthants (the hypervolume case),
  including the full symbolic ``basic function'' machinery of the paper's
  Lemmas~4.4 and~4.5.  We are not aware of any previous implementation of
  this algorithm.
\end{itemize}
...
The recursion maintains a cell $\Gamma$, a list $\bar B$ of surviving orthant
points, and a basic function $F$, and computes
$\int_{\Gamma\setminus\bigcup\bar B} F(x)\,dx$; initially $F\equiv 1$.  The
simplification events are:
\begin{itemize}\itemsep2pt
\item \textbf{Cover.} A box active in no dimension ($y\le \ell$) covers
  $\Gamma$: the node contributes $0$.
\item \textbf{Slabs.} A box active only in dimension $i$ is the halfspace
  $\{x_i\ge y_i\}$; its complement is $\{x_i<y_i\}$, so the cell is shrunk:
  $\Gamma.\mathrm{hi}_i\leftarrow\min(\Gamma.\mathrm{hi}_i,\,y_i)$.  (For
  same-orientation orthants no coordinate collapsing is needed.)
\item \textbf{2-sided orthants.} The boxes active exactly in dimensions
  $\{i,j\}$ are quadrants $\{x_i\ge a_t,\ x_j\ge b_t\}$ in the $(i,j)$-plane.
  The complement of their union is $\{x_j< f(x_i)\}$ where
  $f(t)=\min\{b_t : a_t\le t\}$ is a \emph{single monotone-decreasing step
  function}---the staircase of the 2-D maxima of the points
  (Figure~\ref{fig:staircase}).  Absorption multiplies the condition
  $[x_j\le f(x_i)]$ into every term of $F$; two conditions of the same pair,
...
condition per axis pair, regardless of how many boxes were absorbed.}
\label{fig:staircase}
\end{figure}
...
\subsection{Scope of the specialization}

The implementation restricts Chan's Section~4.2 in one way: all orthants must
share the same orientation (be grounded), which is exactly the hypervolume
setting.  This is what makes the absorbed predicate per axis pair a
\emph{single} decreasing staircase; for mixed orientations the complement of
$\bigcup B_{ij}$ requires up to four monotone step functions
\cite[Sec.~4.1]{Chan2013}, which the term algebra supports but the absorption
front end does not currently produce.  The paper's further results---the
word-RAM speedups of its Section~3 and the $\Oh(n^{(d+1)/3})$ hypercube
algorithm of its Section~4.3---are out of scope.
...
\item \emph{End-to-end}: both algorithms against $\Oh(2^n)$
  ...
  coordinate-compression enumerator.
\end{enumerate}
...
arbitrary boxes as \texttt{(lo, hi)} corner pairs.  The solver classes
...
executable specification against which faster engineered variants (or
implementations of the general-orientation and hypercube cases) can be
checked.

\paragraph{Code availability.}
Both modules, the test suites, and the benchmark scripts are available in the
accompanying repository; everything depends only on the Python standard
library.
